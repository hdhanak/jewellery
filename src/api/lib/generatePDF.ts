...
export const generatePDF = async (htmlCode: any, outputPath: string, response: any): Promise<void> => {
  try {
...
export const getPdf = async (html: any, outputPath: string, response: any): Promise<void> => {
  try {
    ...
    await fs.promises.writeFile(`${outputPath}.html`, finalHtml, 'utf8');

    console.log(__dirname);

    var contentHtml = fs.readFileSync(`${outputPath}.html`, 'utf8');
    ...
    response.download(`${outputPath}.pdf`, "output.pdf", (err: any) => {
      if (err) {
        console.error("Error downloading PDF:", err);
        response.status(500).send("Error downloading PDF");
      } else {
        // Delete the generated PDF file after download
        fs.unlinkSync(`${outputPath}.pdf`);
        fs.unlinkSync(`${outputPath}.html`);
